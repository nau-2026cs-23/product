...
function QuestionDetailModal({ question, onClose }: QuestionDetailModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="text-base font-bold text-slate-900">题目详情</h2>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <h3 className="text-sm font-semibold text-slate-600 mb-2">题目内容</h3>
            <div className="p-3 bg-slate-50 rounded-lg">
              <p className="text-sm text-slate-800 leading-relaxed">{question.content}</p>
            </div>
          </div>
          {question.answer && (
            <div>
              <h3 className="text-sm font-semibold text-slate-600 mb-2">参考答案</h3>
              <div className="p-3 bg-green-50 rounded-lg">
                <p className="text-sm text-green-800 leading-relaxed">{question.answer}</p>
              </div>
            </div>
          )}
          <div className="grid grid-cols-2 gap-3">
            <div>
              <h3 className="text-sm font-semibold text-slate-600 mb-1">题型</h3>
              <p className="text-sm text-slate-800">{QUESTION_TYPE_LABELS[question.questionType as QuestionType] || question.questionType}</p>
            </div>
            <div>
              <h3 className="text-sm font-semibold text-slate-600 mb-1">难度</h3>
              <p className={`text-sm ${DIFFICULTY_COLORS[question.difficulty as DifficultyLevel] || 'text-slate-800'}`}>
                {DIFFICULTY_LABELS[question.difficulty as DifficultyLevel] || question.difficulty}
              </p>
            </div>
          </div>
          {question.knowledgePoint && (
            <div>
              <h3 className="text-sm font-semibold text-slate-600 mb-1">知识点</h3>
              <p className="text-sm text-primary">{question.knowledgePoint}</p>
            </div>
          )}
          <div className="flex gap-3 pt-2">
            <button onClick={onClose} className="flex-1 px-4 py-2.5 text-sm font-medium text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">关闭</button>
          </div>
        </div>
      </div>
    </div>
...
export default function QuestionsView() {
  ...
  return (
    <div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p 
                      ...
                      className="text-sm text-slate-800 leading-relaxed cursor-pointer hover:text-primary transition-colors"
                    >
                      {q.content}
                    </p>
                    {q.answer && (
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    <button onClick={() => setViewQuestion(q)} className="p-1.5 rounded-md hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors" title="查看详情">
      ...
      {viewQuestion && <QuestionDetailModal question={viewQuestion} onClose={() => setViewQuestion(null)} />}
