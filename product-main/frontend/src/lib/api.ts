...
export const uploadApi = {
  getPresignedUrl: (data: { fileName: string; fileType: string; fileSize: number }) =>
    request<{ uploadId: string; presignedUrl: string; s3Key: string; expiresIn: number }>('/api/upload/presigned-url', { 
      method: 'POST', 
      body: JSON.stringify(data) 
    }),

  completeUpload: (data: { uploadId: string; fileName: string; fileSize: number; fileType: string; s3Key: string }) =>
    request<any>('/api/upload/complete', { 
      method: 'POST', 
      body: JSON.stringify(data) 
    }),
};
