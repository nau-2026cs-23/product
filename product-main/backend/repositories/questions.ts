...
export class QuestionRepository {
  ...
  async getStats() {
    ...
    const [totalResult] = await db
      .select({ count: db.count(questions.id) })
      .from(questions);
    ...
    const [wrongResult] = await db
      ...
      .from(questions)
      .where(eq(questions.isWrong, true));
    const wrongCount = wrongResult.count || 0;

    return {
      total,
      wrongCount,
      difficultyStats: Object.fromEntries(
        difficultyStats.map(item => [item.difficulty, Number(item.count)])
      ),
      typeStats: Object.fromEntries(
        typeStats.map(item => [item.type, Number(item.count)])
      )
    };
  }
}
